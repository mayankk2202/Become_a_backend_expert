...
@WebServlet("/ListSt2CMap")
public class ListSt2CMap extends HttpServlet {
	private static final long serialVersionUID = 1L;

	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		System.out.println("Servlet ListSt2CMap called");
		PrintWriter out = response.getWriter();
		HttpSession se = request.getSession(false);
		MastersDAO dao = new MasterDAOImpl();

		if (se != null) {
			Record[] qryRes = dao.ListMappings("St2CMap");
			
			out.println("<!DOCTYPE html><html><head><meta charset=\"ISO-8859-1\"><title>Student to Class Mapping</title>"
					 +"<style>table, th, td {border: 1px solid black;border-collapse: collapse;}</style></head><body>");
			out.println("<div align=\"center\"><h3>The Student to Class Mapping Table is as follows:</h3>");
