...
@WebServlet("/ListT2SMap")
public class ListT2SMap extends HttpServlet {
	private static final long serialVersionUID = 1L;

	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		System.out.println("Servlet ListT2SMap called");
		PrintWriter out = response.getWriter();
		HttpSession se = request.getSession(false);
		MastersDAO dao = new MasterDAOImpl();

		if (se != null) {
			Record[] qryRes = dao.ListMappings("T2SMap");
			
			out.println("<!DOCTYPE html><html><head><meta charset=\"ISO-8859-1\"><title>Teacher to Subject Mapping</title>"
					 +"<style>table, th, td {border: 1px solid black;border-collapse: collapse;}</style></head><body>");
			out.println("<div align=\"center\"><h3>The Teacher to Subject Mapping Table is as follows:</h3>");
			out.println("<table><tr><b><th>Teacher ID</th><th>First Name</th><th>Last Name</th><th> <-- Mapped to --> </th>"
